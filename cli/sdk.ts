...
export class Palmyr {
  ...
  /** Start a browser-extension capture hand-off → token; the connect code the
   *  human pastes into Palmyr Connect is `${this.api}/connect/${token}/session`.
   *  Free, unauthenticated. Used by the default `connect`. */
  async socialTiktokCaptureStart(): Promise<{ token: string; expires_in_sec: number; mode: string }> {
    return this.request('POST', '/social/tiktok/qr', { mode: 'capture' })
  }

  /** Poll for the session the extension posted → { state, captured, cookies }.
   *  Free, token-scoped. */
  async socialTiktokCaptureGet(token: string): Promise<{ state: string; captured: boolean; cookies: any[] | null }> {
    return this.request('GET', `/connect/${token}/session`)
  }
