...
| `palmyr twitter transfer <username> --to <wallet> --confirm` | $0.0001 *(ownership proof; \~$0.0011 with the lookup it does to find the account's server table)* | Atomically hand the X account to another wallet. Works whether the account is **pool-bought** (`x_accounts`) or **BYO-registered** (`social_registered_accounts`); the CLI looks up the username in both tables and dispatches. If the account is only in your local vault (imported, never registered), the command fails with a clear hint to run `palmyr twitter register` first. Server rotates the password and revokes other sessions before flipping ownership, so the local copy of credentials becomes useless. Requires `--confirm`. Local vault entry is removed on success — receiver picks up fresh creds with `palmyr twitter claim`. |
