...
| Command | Cost | Notes |
|---------|------|-------|
| `palmyr tiktok connect <username>` *(`--tag <folder>` files it under a folder)* | free | **Default: extension capture** — the cleanest, most reliable path. Prints a `/connect/<token>` link (in stderr + the result's `login_link`). Your human installs the [Palmyr Connect](../extension/palmyr-connect/) browser extension, logs into the **real tiktok.com** normally, pastes the connect code (`…/connect/<token>/session`), and clicks Connect — the extension reads the session (incl. the HttpOnly `sessionid` via `chrome.cookies`) and posts it to the token-scoped endpoint; the agent polls and saves it. No streamed browser, no proxy, **no TikTok anti-bot to fight** (it's a genuine human login on the real site), works for **any** account incl. email/password. Needs a desktop + a one-time extension install. |
| `palmyr tiktok connect <username> --qr` | free | **Phone-scan hand-off** — prints the `/connect/<token>` link (in `qr_link`); the human scans it with the TikTok app (no password/captcha). The link is **durable (~15 min) and auto-refreshes** as TikTok rotates the code (page polls `/connect/<token>/status`). Best when the account is already in the human's TikTok phone app. |
...
| `palmyr tiktok connect <username> --stream` | free | **Screencast a server browser** to the `/connect/<token>` link (in `login_link`) so a human logs in remotely from any device; clicks/keystrokes replay over CDP, frames relay over HTTP (no VNC). Headed browser required (xvfb on headless). Note: TikTok often blocks typed password login ("Something went wrong") — prefer the default or `--qr`. |
| `palmyr tiktok import <username> --sessionid <s> --csrf <c> --webid <w> --country <iso2>` | free | Bring your own cookies from a logged-in TikTok browser (the zero-install manual version of the default). |
...
ao.socialTiktokHostQr(qrDataUrl?, token?, done?)                  // create/refresh a QR hand-off session → { token, expires_in_sec }; link is `${api}/connect/<token>`. No-arg → new waiting session; pass token+qrDataUrl to refresh the QR; token+done to mark captured
ao.socialTiktokCaptureStart()                                     // start an extension-capture hand-off → { token, expires_in_sec }; connect code is `${api}/connect/<token>/session` (used by the default connect)
ao.socialTiktokCaptureGet(token)                                  // poll for the session the Palmyr Connect extension posted → { state, captured, cookies }
ao.socialTiktokScreenStart()                                      // start a screencast hand-off → { token, expires_in_sec }; link is `${api}/connect/<token>` (used by connect --stream)
ao.socialTiktokScreenFrame(token, frame?, vw?, vh?, done?)        // push the latest browser JPEG frame + drain the human's queued input in one round-trip → { input, state }
