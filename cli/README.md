...
| Command | Cost | Notes |
|---------|------|-------|
| `palmyr tiktok connect <username>` *(`--tag <folder>` files it under a folder)* | free | **Default: QR hand-off** — zero install, the cleanest UX. Prints a `/connect/<token>` link (in stderr + the result's `qr_link`); the human opens it and scans the QR with the **TikTok app on their phone** (where the account is already logged in). No password, no captcha, not sus, never blocked by TikTok. The link is **durable (~15 min) and auto-refreshes** as TikTok rotates the code (page polls `/connect/<token>/status`); the session is captured the moment they confirm. Best when the human **owns** the account. |
| `palmyr tiktok connect <username> --extension` | free | **Desktop-login capture** — for bulk / email-password accounts that aren't on a phone. The human installs the [Palmyr Connect](../extension/palmyr-connect/) extension **once**, logs into the **real tiktok.com**, pastes the connect code (`…/connect/<token>/session`), clicks Connect — the extension reads the session (incl. HttpOnly `sessionid` via `chrome.cookies`) and posts it to the token-scoped endpoint; the agent polls and saves it (in `login_link`). No proxy, no anti-bot, works for **any** account. One install, then connect unlimited accounts. |
