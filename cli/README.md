...
| Command | Cost | Notes |
|---------|------|-------|
| `palmyr tiktok connect <username>` *(`--tag <folder>` files it under a folder)* | free | **Default: human hand-off login (live browser)** — works on any VPS/headless box where you can't touch the browser. Prints a clean `/connect/<token>` link **instantly** (in stderr + the result's `login_link`) to forward to a human; they open it in **any browser on any device** and do the **full email/password/2FA/captcha login** themselves, with their clicks/keystrokes replayed over CDP. The session is captured automatically the moment they're signed in. Pure HTTP relay (frames out via `POST /connect/<token>/frame`, the page polls `/connect/<token>/live` and posts input to `/connect/<token>/input`) — no VNC/remote desktop. Headed browser required (xvfb on headless). |
| `palmyr tiktok connect <username> --qr` | free | **Hand-off via QR instead** — prints the `/connect/<token>` link (in `qr_link`); the human scans it with the TikTok app (no password/captcha). The link is **durable (~15 min) and auto-refreshes** as TikTok rotates the code (page polls `/connect/<token>/status`). Best when the account is already in the human's TikTok phone app. |
| `palmyr tiktok connect <username> --local` | free | **Skip the link** — opens Chrome/Edge/Brave on **this machine** and you log in here yourself; the session is auto-captured via CDP. For a desktop with a human present. No server call. |
...
ao.socialTiktokScreenStart()                                      // start a live-browser (screencast) hand-off → { token, expires_in_sec }; link is `${api}/connect/<token>` (used by connect --remote)
