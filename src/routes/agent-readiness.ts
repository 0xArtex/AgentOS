...
router.get("/agent-readiness", (req: Request, res: Response) => {
  ...
  const quickBootstrap = {
    ...
    steps: [
      { order: 1, action: "Provision phone", curl: `curl -X POST https://palmyr.ai/api/phone/provision -H "X-Agent-Id: ${agentId}"` },
      { order: 2, action: "Send test email", curl: `curl -X POST https://palmyr.ai/api/email/send -H "X-Agent-Id: ${agentId}"` },
      { order: 3, action: "Provision compute", curl: `curl -X POST https://palmyr.ai/api/compute/provision -H "X-Agent-Id: ${agentId}"` },
      { order: 4, action: "Create wallet", curl: `curl -X POST https://palmyr.ai/api/wallet/create -H "X-Agent-Id: ${agentId}"` }
    ]
  };
  ...
  res.json({
    ...
    quickBootstrap,
    pricing: { model: "pay-per-call (x402 USDC)", note: "Each call settles in USDC at request time — see /pricing for exact amounts." },
