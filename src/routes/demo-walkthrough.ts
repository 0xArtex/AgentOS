...
router.get("/demo-walkthrough", (_req: Request, res: Response) => {
  const data = {
    title: "AgentOS Live Demo Walkthrough",
    description: "Step-by-step guide to see AgentOS in action. Each step includes a live curl command.",
    totalSteps: 6,
    ...
    steps: [
      ...
      { step: 4, title: "Send an Email", command: "curl -X POST http://77.42.89.233:3001/api/email/send -H X-Agent-Id:demo-agent", expect: "Email sent through AgentOS relay" },
    ...
    links: { full_docs: "http://77.42.89.233:3001/docs", github: "https://github.com/0xArtex/AgentOS" }
  };
  res.json(data);
});
