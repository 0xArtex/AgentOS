...
export function poolBuy(req: PoolBuyRequest): PoolBuyResult {
  // This shared credential pool is twitter-only. TikTok accounts live in the
  // tiktok_accounts registry (server session = the asset) and are bought via
  // /tiktok/buy, not here.
  if (req.platform !== "twitter") {
    return { success: false, error: "The shared account pool is twitter-only; TikTok accounts are bought via /tiktok/buy." };
  }
  if (!req.buyer_wallet) {
