  ...
  return [
    {
      ...
      text: [
        ...
        "      · twitter_buy_account returns a fully-provisioned account → use the new handle as account_id in subsequent twitter_* steps",
        "  - **Sessions for twitter_*/tiktok_* steps are handled by the executor.** Set `account_id` to the user-supplied handle without the @ (e.g. `'ArianneAgent'` for @ArianneAgent). The executor resolves the handle locally, ensures a fresh login session via the encrypted vault, and injects cookies/credentials at HTTP time. NEVER emit a `twitter_login` or `tiktok_login` step — those capabilities don't exist. NEVER put `cookies`, `login`, `password`, `auth_token`, `ct0`, or `sessionid` in step inputs — those fields are stripped from the catalog because they are vault-only.",
