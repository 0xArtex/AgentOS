  ...
  return [
    {
      ...
      text: [
        ...
        "      · 'send email from my hello@... inbox' / 'from my X inbox' → list_inboxes → send_email (NOT provision_email_inbox). Select the inbox by address with $STEPS.s1.output.inboxes[address=hello@example.com].id — match the user-supplied address against the address field; don't guess an index.",
        ...
        "      · 'post from my X account' → twitter_post directly (the executor handles session/login internally — see Sessions below)",
        "      · 'tweet from a new account' / 'register a fresh domain' / 'spin up a new VPS' → fresh provision is correct",
        "  - **A named resource is presumed owned.** When the user names a specific domain (`example.com`), phone (`+15551234567`), inbox address (`hello@example.com`), or social handle (`@whoever`), assume they ALREADY OWN it unless the intent literally says to register / buy / create / provision a NEW one. Do NOT add a redundant register_domain / provision_phone / etc. step that the user didn't ask for — the destination route's ownership check will catch genuinely-unowned resources cheaply, and tacking on an unrequested register step blows the budget and pollutes the plan. Examples:",
        "      · 'Create an inbox at hello@stealthkicks.xyz' → just provision_email_inbox(name='hello', domain='stealthkicks.xyz'). NOT register_domain → provision_email_inbox.",
        "      · 'Send a tweet from @founder_x' → just twitter_post on the named account. NOT twitter_buy_account → twitter_post.",
        "      · 'Send SMS from +15551234567' → just send_sms with that phone_number_id. NOT provision_phone → send_sms.",
        "      · Counter-example (intent explicitly asks to register): 'Register stealthkicks.xyz and set up an inbox' → register_domain → provision_email_inbox is correct.",
        ...
        "  - **Indexing into list outputs**: list_* capabilities return an array (e.g. list_phones → { numbers: [{ id, ... }] }). To use the first item's id in a downstream step, write $STEPS.sN.output.numbers[0].id. Both `[N]` and `.N` syntaxes are supported. To select an element by ATTRIBUTE instead of position, write $STEPS.sN.output.inboxes[address=hello@example.com].id — the executor returns the first array element whose field equals the value. Use this whenever the user named a specific resource (an inbox address, a domain, a handle) so you don't have to guess its index.",
        ...
        "      · provision_phone returns { id } → use as phone_number_id in send_sms / read_sms / wait_for_otp / start_voice_call / list_calls. For signup/2FA flows that need an SMS verification code, prefer ONE wait_for_otp step (blocks up to 90s, returns the parsed code) over repeated read_sms polling.",
        ...
        "      · twitter_buy_account returns a fully-provisioned account → use the new handle as `handle` in subsequent twitter_* steps",
        "      · twitter_list_my_tweets returns { tweets: [{ tweet_url, tweet_id, posted_at, text }] } sorted oldest-first → use $STEPS.s1.output.tweets[N].tweet_url as tweet_url in twitter_delete_post / twitter_reply / etc. To 'delete all my tweets except the first', emit list_my_tweets at limit=20 then 19 delete_post steps for indices [1..19] (preserving [0] which is the oldest).",
        "  - **Don't over-constrain a pooled-account buy.** For twitter_buy_account, do NOT set `country` or `age_category` unless the user EXPLICITLY asked for one (e.g. 'a US account', 'an aged account'). The pool is thin and accounts are mostly untagged for age, so adding filters the user didn't request usually matches zero and fails the buy. Default to NO filters — buy whatever's available and rebrand it to the brand.",
        "  - **Sessions for twitter_*/tiktok_* steps are handled by the executor.** Set `handle` to the user-supplied handle without the @ (e.g. `'ArianneAgent'` for @ArianneAgent). The executor resolves the handle locally, ensures a fresh login session via the encrypted vault, and injects cookies/credentials at HTTP time. NEVER emit a `twitter_login` or `tiktok_login` step — those capabilities don't exist. NEVER put `cookies`, `login`, `password`, `auth_token`, `ct0`, `sessionid`, or `account_id` in step inputs — those fields are stripped from the catalog because they are vault-only or executor-internal.",
