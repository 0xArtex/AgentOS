  ...
  return async (req: HackathonRequest, res: Response, next: NextFunction): Promise<void> => {
      ...
      if (agent) {
        ...
        res.status(402).json({
          ...
          message: agent.hackathon_verified 
            ? "Free tier exhausted (1 per service). Pay with USDC for additional services."
            : "This service requires USDC payment via x402.",
          hint: "Include X-Payment header with a Solana USDC transaction signature",
          pricing: "GET /pricing",
        });
        return;
      }
    ...
    if (req.headers["x-agent-id"]) {
      ...
      if (agent) {
        ...
        res.status(402).json({
          error: "Payment Required",
          message: "Free tier exhausted or not available. Pay with USDC.",
          hint: "Include X-Payment header with a Solana USDC transaction signature",
        });
        return;
      }
